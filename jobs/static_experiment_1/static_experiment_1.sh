...
# GSAT
msub jobs/gsat.static.template.sh 512 $NAME

# WalkSAT
for i in $WALKSAT;
do
  msub jobs/walksat.static.template.sh 512 $i `echo $NAME`_rho$i;
done;

# ProbSAT
for i in $PROBSAT;
do
  msub jobs/probsat.static.template.sh 512 $i `echo $NAME`_cb$i;
done;
